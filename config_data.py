    ...
    if environment_var_name not in os.environ:
        while environment_var_name not in os.environ:
            user_defined_output_folder = input(
                "Please define a value for '" + environment_var_name + "': (only for this session)\n> ")
            if path.isdir(user_defined_output_folder):
                os.environ[environment_var_name] = str(os.path.abspath(user_defined_output_folder))
                config[environment_var_name[len(env_suffix):]] = str(os.path.abspath(user_defined_output_folder))
            else:
