...
class FigureExtractor:
    ...
    def get_figure_data(self):
        ...
        for file in os.listdir(figureDataPath):
            ...
            if len(prunedJson) != 0:
                ...
                newfile = open(
                    os.path.join(filePath, pdfID + ".json"), "w", encoding="utf-8"
                )
    ...
    def labelFigures(self, max_labels):
        ...
        figureDataPath = self.outputPath
        for file in os.listdir(figureDataPath):
            pdfID = self.get_filename(file)
            print(pdfID)
...
if __name__ == "__main__":
    figureExtractor = FigureExtractor(
        "IO/INPUT_ALL", "IO/OUTPUT_FIG", "IO/OUTPUT_DATA", "../OUTPUT_PRUNED"
    )
