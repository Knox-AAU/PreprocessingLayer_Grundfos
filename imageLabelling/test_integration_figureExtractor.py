...
def testFileName():
    ...
    figureExtractor = FigureExtractor(
        "tests/test_json",
        "tests/TEST_OUTPUT_FIG",
        "tests/TEST_OUTPUT_DATA",
        "tests/TEST_OUTPUT_PRUNED",
        "",
    )
    ...
    # assert
    with open(
        "tests/TEST_OUTPUT_PRUNED/Grundfosliterature-2405.json", encoding="utf8"
    ) as actualData:
        actualJSON = json.load(actualData, encoding="utf8")
        actualJSON = json.dumps(actualJSON, sort_keys=True)

    with open(
        "tests/test_json/Grundfosliterature-2405.json", encoding="utf8"
    ) as expectedData:
