...
class TextAnalyser:
    ...
    def find_sections(self, PDF):
        ...
        while self.__CurrentText__ != None:
            ...
            if self.is_simular_size_in_list(
                self.__CurrentTextFontSize__, self.__BlockTextFontSizes__, 0.01
            ):
    ...
    def find_subsections(self, CurrentSection, TitleFontSize):
        while self.__CurrentText__ != None:
            if self.is_simular_size_in_list(
                ...
                self.is_simular_font_size(
                    self.__CurrentTextFontSize__, TitleFontSize, 0.01
                )
                or TitleFontSize < self.__CurrentTextFontSize__
            ):
    ...
    def next_text(self):
        ...
        if self.__FollowingLine__ != None:
            ...
            while self.is_simular_font_size(
                self.__CurrentTextFontSize__,
                self.font_size(self.__FollowingLine__),
                0.01,
            ):
                self.__CurrentText__ += self.__FollowingLine__.get_text()
                self.next_line()
    ...
    def next_line(self):
        ...
        if self.__NextLineIndex__ < len(self.__Pages__[self.__PageIndex__]):
            self.__FollowingLine__ = self.__Pages__[self.__PageIndex__][
                self.__NextLineIndex__
            ]
            ...
            if self.__PageIndex__ < len(self.__Pages__):
                self.__BlockTextFontSizes__ = self.find_block_text_font_sizes(
                    self.__Pages__[self.__PageIndex__]
                )
                self.__NextLineIndex__ = 0
                self.next_line()
            else:
    ...
    def find_title(self, FirstPage):
        try:
            ...
            for line in FirstPage[LargestColumnIndex]:
                if self.is_simular_font_size(
                    self.font_size(line), LargestFontSize, 0.3
                ):
    ...
    def find_columns(self, Page):
        ...
        if len(Page) != 0:
            ...
            for x in range(1, len(Page)):
                ...
                HoriDist = self.CoordsCalc.compare_horizontal_dist(
                    Page[x], ColumnCoords
                )
                ...
                if VertDist > 0 or VertDist < LastFontSize * -2.5 or HoriDist != 0:
                    ...
                    ColumnLines = [Page[x]]
                    ColumnCoords = self.CoordsCalc.convert_object_to_coordinates(
                        Page[x]
                    )
        ...
        while len(Columns) != 0:
            ...
            for x in range(1, len(Columns)):
                ...
                # tests if the current column is futher up on the page then the selected
                    ...
                    self.CoordsCalc.compare_vertical_dist(
                        Columns[x][0], SelectedColumnCoords
                    )
                    > 0
                ):
                    SelectedColumnCoords = Columns[x][0]
