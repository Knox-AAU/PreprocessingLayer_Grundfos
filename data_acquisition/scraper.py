...
class Scraper:
    ...
    PATH_TO_INVALID_LINKS = os.path.join(
        pathlib.Path(__file__).parent.absolute(), "scraperdata", "invalid_links.txt"
    )
    PATH_TO_VALID_LINKS = os.path.join(
        pathlib.Path(__file__).parent.absolute(),
        "scraperdata",
        "downloadable_links.txt",
    )
    PATH_TO_INDEXES_CHECKED = os.path.join(
        pathlib.Path(__file__).parent.absolute(), "scraperdata", "checked_links.txt"
    )
    ...
    def read_index_from_file(self):
        file = open(self.PATH_TO_INDEXES_CHECKED, "a+")
        file.seek(0)
        try:
