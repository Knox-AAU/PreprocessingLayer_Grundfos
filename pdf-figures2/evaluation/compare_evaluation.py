...
def main():
    ...
    parser.add_argument(
        "-d", "--documents", nargs="+", help="Only compare on these documents"
    )
    ...
    if eval1.dataset_name != eval2.dataset_name:
        raise ValueError(
            "Evaluations had different datasets (%s vs %s)"
            % (eval1.dataset_name, eval2.dataset_name)
        )
    if eval1.dataset_version != eval2.dataset_version:
        print(
            "WARNING: Evaluation compared different dataste versions (%s vs %s)"
    ...
    print(
        "Evaluations shared %d docs (%d in eval1, %d in eval2)"
        % (len(same_docs), len(eval1.docs), len(eval2.docs))
    )
    ...
    for fig in eval2.evaluated_figures:
        if fig.doc in same_docs:
            ...
            if fig.get_id() not in eval1_figs:
                differences += 1
                print(
                    "Eval2 has %s: %s %s but Eval1 does not"
                    % (fig.doc, fig.name, fig.error)
                )
    ...
    for fig_id, figs in eval1_figs.items():
        ...
        if fig_id not in eval2_figs:
            ...
            for fig in figs:
                print(
                    "Eval1 has %s: %s %s but Eval2 does not"
                    % (fig.doc, fig.name, fig.error)
                )
        else:
            ...
            if len(no_matches) == 1 and len(no_matches_other) == 1:
                ...
                print(
                    ...
                    % (
                        doc,
                        name,
                        str([x.error.name for x in no_matches]),
